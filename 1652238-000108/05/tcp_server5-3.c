...
int main(int argc, char const *argv[])
{
	...
	int opt = 2000;
	if (setsockopt(server_fd, SOL_SOCKET, SO_RCVBUF , &opt, sizeof(opt)))
	{
		perror("setsockopt");
		exit(EXIT_FAILURE);
	}
	else
	{
		...
		struct linger l;
		optlen = sizeof(int);
		rc = getsockopt(server_fd, SOL_SOCKET, SO_RCVBUF, (char *)&optval, &optlen);
		if (rc == 0)
		{
	...
	while (1)
	{
		...
		while(1)
		{
			...
			readbytes += 1;
			printf("read %d bytes, %d total\n",sizeof(buffer),sizeof(buffer)*readbytes);
			getchar();
